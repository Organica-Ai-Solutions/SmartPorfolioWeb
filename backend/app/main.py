    ...
    try:
        ...
        try:
            ...
            if (end_date - start_date).days < min_data_days:
                print(
                    f"Start date too recent, adjusting to get at least {min_data_days} days of data"
                )
        ...
        crypto_tickers = [
            ...
            if crypto_service.is_crypto_ticker(ticker)
        ]
        stock_tickers = [
            ticker for ticker in request.tickers if ticker not in crypto_tickers
        ]
        ...
        if crypto_tickers:
            ...
            try:
                crypto_data = crypto_service.get_crypto_price_history(
                    crypto_tickers, start_date, end_date
                )
                if not crypto_data.empty:
                    ...
                    print(
                        "No crypto data available, will try Yahoo Finance as fallback"
                    )
            except Exception as e:
        ...
        if stock_tickers or combined_data.empty:
            ...
            tickers_set = set()
            unique_tickers = [
                t for t in fixed_tickers if not (t in tickers_set or tickers_set.add(t))
            ]
            print(
                f"Using expanded ticker list for more reliable data: {unique_tickers}"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(
                        f"No data downloaded. Retrying {attempt + 1}/{max_retries}..."
                    )
        ...
        if combined_data.empty or len(combined_data.columns) == 0:
            print(
                "Yahoo Finance and CoinMarketCap failed to provide data, trying alternative sources"
            )
            ...
            if not alt_data.empty and len(alt_data.columns) > 0:
                ...
                print(
                    "Alternative sources also failed, falling back to simple analysis"
                )
        ...
        if combined_data.empty or len(combined_data.columns) == 0:
            print(
                "No data available after all attempts, falling back to simple analysis"
            )
            return await analyze_portfolio_simple(request)
    ...
    try:
        ...
        def generate_ticker_metrics(ticker):
            ...
            return {
                ...
                "var_95": -volatility_factor
                * 1.65
                / math.sqrt(252),  # Simplified VaR calculation
                "max_drawdown": max_drawdown,
                "correlation": 0.3 + (seed % 0.5),  # Correlation between 0.3 and 0.8
                "sharpe_ratio": (
                    (return_factor - 0.02) / volatility_factor
                    if volatility_factor > 0
                    else 0
                ),
                "sortino_ratio": (
                    ((return_factor - 0.02) / volatility_factor) * 1.2
                    if volatility_factor > 0
                    else 0
                ),
            }
        ...
        asset_metrics = {
            ticker: generate_ticker_metrics(ticker) for ticker in request.tickers
        }
        ...
        portfolio_return = sum(
            metrics["annual_return"] * optimized_weights[ticker]
        ...
        portfolio_volatility = sum(
            metrics["volatility"] * optimized_weights[ticker]
            for ticker, metrics in asset_metrics.items()
        )

        # Calculate other portfolio metrics
        portfolio_beta = sum(
            metrics["beta"] * optimized_weights[ticker]
            for ticker, metrics in asset_metrics.items()
        )
        portfolio_sharpe = (
        ...
        portfolio_max_dd = (
            min(metrics["max_drawdown"] for metrics in asset_metrics.values()) * 0.8
        )  # Simplified
        ...
        relative_performance = [
            (p / s - 1) * 100 for p, s in zip(portfolio_values, sp500_values)
        ]
        ...
        return {
            ...
            "discrete_allocation": {
                "shares": {
                    ticker: int(10000 * optimized_weights[ticker] / 100)
                    for ticker in request.tickers
                },
                "leftover": 1000.0,
            },
...
async def rebalance_portfolio(allocation: PortfolioAllocation):
    try:
        ...
        if not allocation.allocations or not isinstance(allocation.allocations, dict):
            raise HTTPException(
                status_code=400,
                detail="Invalid allocation data: allocations must be a non-empty dictionary",
            )
        ...
        for ticker, weight in allocation.allocations.items():
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid weight value for {ticker}"
                )
        ...
        if allocation.use_paper_trading:
            api_key = allocation.alpaca_api_key or os.getenv("ALPACA_PAPER_API_KEY")
            secret_key = allocation.alpaca_secret_key or os.getenv("ALPACA_PAPER_SECRET_KEY")
        else:
        ...
        try:
            # Set the correct base URLs for paper/live trading
            if use_paper:
            ...
            print(f"Initializing Alpaca client with base URL: {base_url}")
            print(f"Using API key starting with: {api_key[:4]}...")
            
            # Initialize trading client with proper URLs and raw_data=True
            ...
            try:
                ...
                # Check if we got a dictionary response (error case)
                if isinstance(account, dict):
                    if "message" in account:
                        print(f"Account error response: {account}")
                        raise HTTPException(
                            status_code=401,
                            detail=f"Failed to authenticate with Alpaca API: {account['message']}"
                        )
                    
                # Verify we can access account properties
                try:
                    equity = float(account.equity)
                    print(f"Successfully connected. Account equity: ${equity}")
                except AttributeError as attr_err:
                    print(f"Error accessing account properties: {str(attr_err)}")
                    print(f"Account response type: {type(account)}")
                    print(f"Account response: {account}")
                    raise HTTPException(
                        status_code=401,
                        detail="Failed to access account information. Please check your API keys."
                    )
                ...
                raise HTTPException(
                    status_code=401,
                    detail=f"Failed to authenticate with Alpaca API: {str(auth_error)}"
                )
            ...
            try:
                ...
                for symbol, target_value in target_positions.items():
                    ...
                    if (
                        abs(difference)
                        > (allocation.rebalance_threshold or 0.05) * target_value
                    ):
                        # Get current price
                        ...
                        if shares > 0:
                            ...
                            try:
                                ...
                                print(
                                    f"Error placing order for {symbol}: {str(order_error)}"
                                )
                                trades.append(
                                    {
                                        "symbol": symbol,
                ...
                raise HTTPException(
                    status_code=500, detail=f"Error during trading: {str(e)}"
                )

        except Exception as e:
            print(f"Error during rebalancing: {str(e)}")
            raise HTTPException(
                status_code=500, detail=f"Error during rebalancing: {str(e)}"
            )
...
async def rebalance_portfolio_simple(allocation: PortfolioAllocation):
    """Rebalance portfolio using Alpaca trading with enhanced error handling and features."""
    logging.info("Starting portfolio rebalancing operation")
    try:
        # Validate allocations exist and are not empty
        if not allocation.allocations or not isinstance(allocation.allocations, dict):
            logging.error("Invalid allocation data provided")
            raise HTTPException(
                status_code=400,
                detail="Invalid allocation data: allocations must be a non-empty dictionary",
            )
        ...
        for ticker, weight in allocation.allocations.items():
            try:
                weight_float = float(weight)
                if not (0 <= weight_float <= 1):
                    logging.error(f"Invalid weight for {ticker}: {weight_float}")
                    raise ValueError(f"Weight for {ticker} must be between 0 and 1")
                formatted_allocations[ticker] = weight_float
                total_allocation += weight_float
            except (ValueError, TypeError) as e:
                logging.error(f"Error processing weight for {ticker}: {str(e)}")
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid weight value for {ticker}: {str(e)}",
                )

        # Validate total allocation with more precise error message
        if not (0.99 <= total_allocation <= 1.01):
            logging.error(f"Invalid total allocation: {total_allocation}")
            raise HTTPException(
                status_code=400,
                detail=f"Total allocation must sum to 1 (current sum: {total_allocation:.4f})",
            )

        # Get API keys with enhanced validation
        api_key = (
            allocation.alpaca_api_key
            or os.getenv("ALPACA_PAPER_API_KEY")
            or os.getenv("ALPACA_API_KEY")
        )
        secret_key = (
            allocation.alpaca_secret_key
            or os.getenv("ALPACA_PAPER_SECRET_KEY")
            or os.getenv("ALPACA_SECRET_KEY")
        )
        use_paper = (
            allocation.use_paper_trading
            if allocation.use_paper_trading is not None
            else True
        )
        ...
        # Initialize Alpaca client with enhanced error handling
        try:
            base_url = "https://paper-api.alpaca.markets" if use_paper else "https://api.alpaca.markets"
            logging.info(
                f"Initializing Alpaca client with {'paper' if use_paper else 'live'} trading"
            )
            ...
            logging.info(
                f"Account connected successfully. Equity: ${equity:.2f}, Buying Power: ${buying_power:.2f}"
            )

            # Additional validation for account status
            if account.trading_blocked:
                raise HTTPException(
                    status_code=400, detail="Trading is blocked on this account"
                )
            if account.account_blocked:
                raise HTTPException(status_code=400, detail="Account is blocked")

        except Exception as e:
            logging.error(f"Failed to initialize Alpaca client: {str(e)}")
            raise HTTPException(
                status_code=401, detail=f"Failed to connect to Alpaca API: {str(e)}"
            )
        ...
        try:
            ...
            logging.info(
                f"Current positions retrieved: {len(current_positions)} positions found"
            )

            # Calculate target positions with validation
            target_positions = {
                symbol: total_value * weight
                for symbol, weight in formatted_allocations.items()
            }

            # Enhanced validation for minimum trade size
            ...
            for symbol, target_value in target_positions.items():
                try:
                    ...
                    # Skip if difference is within threshold
                    threshold = (allocation.rebalance_threshold or 0.05) * target_value
                    if abs(difference) <= threshold:
                        logging.info(
                            f"Skipping {symbol} - difference ({difference:.2f}) within threshold ({threshold:.2f})"
                        )
                        continue
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt == max_retries - 1:
                                raise price_error
                            logging.warning(
                                f"Retry {attempt + 1}/{max_retries} getting price for {symbol}"
                            )
                            time.sleep(retry_delay)
                            retry_delay *= 2

                    # Calculate shares with minimum trade value check
                    shares = abs(int(difference / price))
                    ...
                    if trade_value < min_trade_value:
                        skipped_trades.append(
                            {
                                "symbol": symbol,
                                "reason": f"Trade value (${trade_value:.2f}) below minimum (${min_trade_value:.2f})",
                            }
                        )
                        continue

                    # Validate against buying power for buys
                    if difference > 0 and trade_value > buying_power:
                        skipped_trades.append(
                            {
                                "symbol": symbol,
                                "reason": f"Insufficient buying power (${buying_power:.2f}) for trade (${trade_value:.2f})",
                            }
                        )
                        continue
                    ...
                    # Create market order with additional parameters
                    order_data = MarketOrderRequest(
                        symbol=symbol,
                        qty=shares,
                        side=side,
                        time_in_force=TimeInForce.DAY,
                        client_order_id=f"rebalance_{symbol}_{int(time.time())}",  # Add unique identifier
                    )
                    ...
                    try:
                        ...
                        logging.info(
                            f"Order submitted for {symbol}: {shares} shares {side.value}"
                        )
                    except Exception as order_error:
                        logging.error(
                            f"Error placing order for {symbol}: {str(order_error)}"
                        ...
                        skipped_trades.append(
                            {
                                "symbol": symbol,
                                "reason": f"Order submission failed: {str(order_error)}",
                            }
                        )

                except Exception as symbol_error:
                    logging.error(f"Error processing {symbol}: {str(symbol_error)}")
                    skipped_trades.append(
                        {
                            "symbol": symbol,
                            "reason": f"Processing error: {str(symbol_error)}",
                        }
                    )
            ...
            response = {
                ...
                "rebalance_summary": {
                    ...
                    "skipped_orders": len(skipped_trades),
                    "completion_time": datetime.now().isoformat(),
                },
            }

            logging.info(
                f"Rebalancing completed: {len(trades)} orders placed, {len(skipped_trades)} trades skipped"
            )
            return response

        except Exception as e:
            logging.error(f"Error during trading operations: {str(e)}")
            raise HTTPException(
                status_code=500, detail=f"Error during trading operations: {str(e)}"
            )

    except HTTPException as he:
        raise he
    except Exception as e:
        logging.error(f"Unexpected error during rebalancing: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Unexpected error during rebalancing: {str(e)}"
        )
...
async def ai_sentiment_analysis(portfolio: Portfolio):
    ...
    try:
        ...
        for i, ticker in enumerate(portfolio.tickers):
            ...
            if is_crypto_asset:
                ...
                selected_factors = random.sample(
                    key_factors, k=min(3, len(key_factors))
                )
                ...
                base_sentiment = (
                    "positive"
                    if i % 3 != 0
                    else "neutral" if i % 3 == 1 else "negative"
                )
                sentiment_data["ticker_sentiments"][ticker] = {
                    "sentiment": base_sentiment,
        ...
        if sentiment_data["ticker_sentiments"]:
            avg_score = sum(
                t["score"] for t in sentiment_data["ticker_sentiments"].values()
            ) / len(sentiment_data["ticker_sentiments"])
    ...
    try:
        ...
        analysis_result = {
            "ai_insights": {
                ...
                "sentiment_scores": {
                    ticker: round(0.5 + 0.3 * random.random(), 2)
...
async def ai_rebalance_explanation(allocation: PortfolioAllocation):
    ...
    try:
        ...
        # Analyze risk impact
        current_weights = {
            k: v / total_value for k, v in rebalance_result["current_positions"].items()
        }
        target_weights = {
            k: v / total_value for k, v in rebalance_result["target_positions"].items()
        }

        insights["risk_impact"] = {
            "diversification": (
                "Improved"
                if len(target_weights) > len(current_weights)
                else "Maintained"
            ),
            ...
            "risk_metrics": {
                ...
                "after": {
                    "concentration": (
                        max(target_weights.values()) if target_weights else 0
                    ),
                },
            },
        }
        ...
        insights["cost_analysis"] = {
            ...
            "estimated_impact": (
                "Low" if total_trades < 5 else "Medium" if total_trades < 10 else "High"
            ),
            "trading_efficiency": (
                "Optimal" if total_trades < 5 else "Consider consolidating trades"
            ),
        }
        ...
        # Combine results
        result = {**rebalance_result, "ai_insights": insights}
        ...
        raise HTTPException(
            status_code=500, detail=f"Error in AI rebalance explanation: {str(e)}"
        )
    ...
    for ticker in tickers:
        try:
            ...
            if ticker in ["GDPC1", "DGS10", "UNRATE", "CPIAUCSL", "VIXCLS"]:
                ...
                try:
                    ticker_data = web.DataReader(
                        ticker, "fred", start=start_date, end=end_date
                    )
                    if not ticker_data.empty:
